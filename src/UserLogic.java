...
public class UserLogic {
	...
	public void addUser(String name, double startingBalance) {
		if (!users.containsKey(name)) {
			users.put(name, new User(name, startingBalance));
			saveUsers();
		}
	}
	
	// Adds an expense to an user
	public boolean addExpenseToUser(User user, String category, double amount, String description) {
		if (user != null) {
			user.addExpense(category, amount, description);
			saveUsers();
			return true;
		}
		return false;
	...
	// Deletes an user of the map and saves the change
	...
	// Deletes an expense from an user according to the description and adds the amount back to the user
	...
	// Deletes an income from an user according to the description and subtracts the amount from the user
